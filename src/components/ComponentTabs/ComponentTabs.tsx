...
export default function ComponentTabs({ children }: TabsProps) {
    ...
    return (
        <div className={styles.componentTabs}>
            <div className={styles.componentTabs__header}>
                <ul className={styles.componentTabs__list} ref={headerListRef} role="tablist">
                    {children.map(
                        (el, idx) =>
                            React.isValidElement(el) && (
                                <li key={idx} className={styles.componentTabs__item}>
                                    <button
                                        id={`tab-${idx}`}
                                        className={`${styles.componentTabs__btn} ${idx === 0 ? "active" : ""}`}
            ...
            <div className={styles.componentTabs__content} ref={contentRef}>
                {children.map(
                    (item, idx) =>
                        React.isValidElement(item) && (
                            <div
                                key={idx}
                                id={`tabpanel-${idx}`}
                                className={`${styles.componentTabs__wrapper} ${idx === 0 ? "show" : ""}`}
